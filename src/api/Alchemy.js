...
const AlchemyAPI = ({searchAddress, passNftData}) => {
  ...
  useEffect(() => {
    ...
    nftData.map(({contract}) => {
      if(!contracts.includes(contract.address)) {
        return contracts.push(contract.address);
      } else {
        return null;
      } 
    });
  ...
  //HANDLES IMAGE URLS
  const parseImgUrl = ({rawMetadata, title, tokenId}) => {
    let url = rawMetadata.image || rawMetadata.image_url;
  ...
  return (
    <div className='cards'>
      ...
      <div className='nft-wrapper'>
        {
          nftData.map((data) => (
            <div className='card-content' key={data.tokenId}>
              {parseImgUrl(data)}
              <div className='top-card-wrapper overlay'>
                <div className='background-shade'>
                  <div className='nft-title'>{data.title}</div>
                  <div className='opensea'>
                    <a className="opensea-link" href={`https://opensea.io/assets/${data.contract.address}/${data.tokenId}`} target="_blank" rel="noreferrer">View on OpenSea</a>
